  ...
  if (q.isError) {
    return (
      <SafeAreaView style={styles.root} edges={['top', 'left', 'right']}>
        <StackScreenHeader title={t('settings.accountDeletionTitle')} />
        ...
        <ScrollView contentContainerStyle={styles.list}>
          ...
          {st.requestedAt ? (
            <Text style={styles.para}>
              {t('settings.deletionRequestedAt', { at: st.requestedAt.slice(0, 10) })}
            </Text>
          ) : null}
          {st.scheduledFor ? (
            <Text style={styles.para}>
              {t('settings.deletionScheduled', { at: st.scheduledFor.slice(0, 10) })}
            </Text>
          ) : null}
          ...
          <Pressable
            style={({ pressed }) => [styles.btn, (cancel.isPending || req.isPending) && styles.disabled, pressed && styles.pressed]}
            onPress={() => {
    ...
    <SafeAreaView style={styles.root} edges={['top', 'left', 'right']}>
      ...
      <ScrollView contentContainerStyle={styles.list} keyboardShouldPersistTaps="handled">
        ...
        <Pressable
          ...
          onPress={() => {
            Alert.alert(
              t('settings.deletionConfirmTitle'),
              t('settings.deletionConfirmBody'),
              [
