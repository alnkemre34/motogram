...
export function EmergencyContactsScreen() {
  ...
  const add = useMutation({
    mutationFn: () => createEmergencyContact({ name: name.trim(), phone: phone.trim(), relationship: relationship.trim() || undefined }),
    onSuccess: () => {
      setName('');
      setPhone('');
      setRelationship('');
  ...
  if (q.isError) {
    return (
      <SafeAreaView style={styles.root} edges={['top', 'left', 'right']}>
        <StackScreenHeader title={t('settings.emergencyTitle')} />
    ...
    <SafeAreaView style={styles.root} edges={['top', 'left', 'right']}>
      ...
      <ScrollView contentContainerStyle={styles.list} keyboardShouldPersistTaps="handled">
        ...
        {add.isError || del.isError ? (
          <Text style={styles.err}>{t('settings.emergencyError')}</Text>
        ) : null}
        {list.length === 0 ? (
        ...
        <View style={styles.field}>
          ...
          <TextInput value={relationship} onChangeText={setRelationship} style={styles.input} placeholderTextColor="#666" />
        </View>
        <Pressable
        ...
        >
          {add.isPending ? (
            ...
            <Text style={styles.addText}>{t('settings.emergencyAddCta')}</Text>
          )}
        </Pressable>
      </ScrollView>
    </SafeAreaView>
