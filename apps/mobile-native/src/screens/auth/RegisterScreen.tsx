...
export function RegisterScreen({ navigation }: Props) {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      navigation.dispatch(
        CommonActions.reset({
          index: 0,
          routes: [{ name: 'Login', params: { postRegisterHint: true } }],
        }),
      );
    },
  });
  ...
  return (
    <View style={styles.root}>
      ...
      />
      {fieldMsg('eulaAccepted') ? <Text style={styles.fieldError}>{fieldMsg('eulaAccepted')}</Text> : null}

      {/* Social auth: requires native Apple/Google SDK wiring + backend OAuth DTOs. */}
      ...
      <Pressable
        ...
        disabled={mutation.isPending || !eulaWatch}
      >
        {mutation.isPending ? <ActivityIndicator color="#fff" /> : <Text style={styles.primaryText}>{t('auth.register.submit')}</Text>}
      </Pressable>
