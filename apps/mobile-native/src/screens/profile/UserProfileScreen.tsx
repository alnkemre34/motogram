...
export function UserProfileScreen({ route, navigation }: Props) {
  ...
  const followM = useMutation({
    ...
    onSuccess: () => {
      const id = userQ.data?.id;
      if (id) {
        void qc.invalidateQueries({ queryKey: ['follow-status', id] });
        void qc.invalidateQueries({ queryKey: ['user', 'public', username] });
  ...
  const showActions = !isMe && Boolean(meQ.data);
    ...
    <SafeAreaView style={styles.root} edges={['top', 'left', 'right']}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.header}>
          ...
          {p.isPrivate ? <Text style={styles.lock}>{t('userProfile.private')}</Text> : null}
        </View>
...
const styles = StyleSheet.create({
  ...
  actions: { gap: 12 },
  primaryBtn: { backgroundColor: '#ff6a00', paddingVertical: 14, borderRadius: 12, alignItems: 'center' },
