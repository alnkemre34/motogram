...
export function DiscoverScreen() {
  ...
  const coords = fg.fix ? { lat: fg.fix.lat, lng: fg.fix.lng } : null;
  ...
  const listData: CommunitySummary[] = useMemo(() => {
    if (searchEnabled) return searchQ.data?.items ?? [];
    if (tab === 'nearby') {
  ...
  const loading = searchEnabled ? searchQ.isPending : tab === 'nearby' ? Boolean(coords) && nearbyQ.isPending : mineQ.isPending;
  const refreshing = searchEnabled ? searchQ.isFetching : tab === 'nearby' ? nearbyQ.isFetching : mineQ.isFetching;
  ...
  const showNearbyHint = !searchEnabled && tab === 'nearby' && (fg.status === 'denied' || !coords);

  useEffect(() => {
    if (tab !== 'nearby') return;
    if (fg.status !== 'denied') return;
    // Keep it silent; the hint text already informs the user.
  }, [tab, fg.status]);
      ...
      {isError ? (
        ...
        <View style={styles.center}>
          <Text style={styles.muted}>
            {fg.status === 'denied' ? t('discover.locationDenied') : t('discover.locationPending')}
          </Text>
        </View>
      ) : (
        <FlatList
          ...
          ListEmptyComponent={
            <Text style={styles.muted}>{searchEnabled ? t('discover.emptySearch') : t('discover.empty')}</Text>
          }
          renderItem={({ item }) => (
            <Pressable
            ...
            >
              ...
              <Text style={styles.cardMeta}>
                {t('discover.memberCount', { n: item.membersCount })} · {t(`community.visibility.${item.visibility}`)}
              </Text>
            </Pressable>
          )}
        />
